  ...
  return <main id="main" className="fixed inset-0 z-[100] overflow-y-auto bg-[#111311] text-white">
    <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-12">
      <header className="flex flex-wrap items-end justify-between gap-6 border-b border-white/15 pb-8">
        <div><p className="eyebrow text-[var(--brand)]">Protected project media</p><h1 className="mt-3 text-4xl font-black tracking-[-.04em] sm:text-6xl">FLOWCOAT Media Studio</h1><p className="mt-3 max-w-2xl text-white/65">Upload approved project photography directly into the website gallery. Production access is password protected and media is stored in the configured persistent media directory.</p></div>
      ...
      <section className="grid gap-8 py-10 lg:grid-cols-[.8fr_1.2fr]">
        <form onSubmit={upload} className="h-fit border border-white/15 bg-white/[.04] p-6 sm:p-8">
          ...
          <div className="mt-6 border-t border-white/10 pt-5 text-xs leading-relaxed text-white/40">Media Studio uploads are separate from GitHub commits. Repository images are read-only here.</div>
        </form>
        ...
        <div>
          ...
          {items.length === 0 ? <div className="border border-dashed border-white/20 p-10 text-center text-white/50">No project images yet.</div> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => <article key={`${item.source}:${item.name}`} className="overflow-hidden border border-white/10 bg-white/[.04]"><div className="relative aspect-[4/3] bg-black/30"><Image src={item.url} alt="" fill unoptimized={item.source === "local"} sizes="(max-width: 640px) 100vw, 33vw" className="object-cover"/></div><div className="flex items-center justify-between gap-3 p-3"><div className="min-w-0"><p className="truncate text-xs font-bold">{item.name.replace(/^\d+-[a-f0-9]+-/, "")}</p><p className="mt-1 text-[.65rem] uppercase tracking-[.16em] text-white/35">{item.source === "local" ? "Media Studio upload" : "Repository asset"}</p></div>{item.source === "local" && <button type="button" disabled={busy} onClick={() => void remove(item)} className="shrink-0 text-xs font-bold text-[var(--brand)] disabled:opacity-40">Delete</button>}</div></article>)}</div>}
        </div>
      </section>
    </div>
  </main>;

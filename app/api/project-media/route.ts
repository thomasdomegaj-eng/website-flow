...
export async function GET(request: Request) {
  const collection = parseMediaCollection(
...
export async function POST(request: Request) {
  if (!localMediaStudioEnabled()) {
    return NextResponse.json(
      { error: "Media Studio is unavailable." },
      { status: 404 }
    );
  }
  ...
  // Windows IIS may reject the HTTP DELETE verb before Next.js sees it.
  // JSON POST requests are therefore used for Media Studio deletion.
  if (contentType.includes("application/json")) {
    ...
    return NextResponse.json({
      ok: true,
      items: await listProjectMedia(undefined, collection),
    });
  }
